@props([
    // sm (default) | xs | lg — controls the glyph size.
    'size' => 'sm',
])
...
{{-- Verified-customer badge: a solid blue check-badge (the check is punched out
     via even-odd fill so it reads white). Shown next to a KYC-verified customer's
     name everywhere it appears. --}}
<span {{ $attributes->merge(['class' => 'inline-flex shrink-0 items-center']) }} title="Verified customer" aria-label="Verified customer">
    <svg class="{{ $dim }} text-blue-600" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">

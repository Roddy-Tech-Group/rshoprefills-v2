...
@if ($status->value === 'completed')
    ...
    <div class="min-h-full bg-zinc-50 dark:bg-[#0c1a36]">
        <div class="mx-auto w-full max-w-sm px-4 py-8 sm:py-10">
            ...
            {{-- Native review prompt. Clicking a star sets the rating and reveals
                 an inline form (no Trustpilot redirect). The review is saved
                 unpublished for admin approval, then shows on the storefront
                 reviews wall once approved. --}}
            <div
                    ...
                    async submit() {
                        ...
                        try {
                            const res = await fetch('{{ route('reviews.store') }}', {
                                ...
                                body: JSON.stringify({ author_name: this.name, body: this.body, rating: this.rating }),
                            });
            ...
            >
                ...
                <div x-show="! submitted" class="mt-5 flex items-center justify-center gap-3" @mouseleave="hover = 0">
                    <template x-for="i in 5" :key="i">
                        <button type="button" @click="rating = i; open = true" @mouseenter="hover = i" :aria-label="`Rate ${i} out of 5`" class="transition-transform duration-200 hover:scale-110">
                            <svg class="h-9 w-9 transition-colors duration-200" :class="(hover || rating) >= i ? 'fill-blue-600 stroke-blue-600 dark:fill-blue-400 dark:stroke-blue-400' : 'fill-transparent stroke-zinc-300 dark:stroke-zinc-600'" viewBox="0 0 24 24" stroke-width="1.5" stroke-linejoin="round" aria-hidden="true">
                ...
                <div x-show="open && ! submitted" x-collapse x-cloak class="mx-auto mt-5 max-w-md text-left">
                    ...
                    <input x-model="name" type="text" maxlength="80" class="mt-1.5 w-full rounded-[10px] border border-zinc-200 bg-white px-3 py-2.5 text-sm text-zinc-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15 dark:border-zinc-700/60 dark:bg-[#1d3252] dark:text-white">

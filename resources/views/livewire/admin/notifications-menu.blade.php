...
>
    ...
    >
        ...
        @if ($this->unreadCount > 0)
            <span class="pointer-events-none absolute -top-0.5 -right-0.5 inline-flex">
                ...
                <span class="relative inline-flex h-5 min-w-[20px] items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-bold text-white">{{ $this->unreadCount > 99 ? '99+' : $this->unreadCount }}</span>
            </span>
        @endif
    </button>
    ...
    <div
        ...
        class="absolute right-0 top-full z-50 mt-2 w-[340px] max-w-[calc(100vw-2rem)] overflow-hidden rounded-xl bg-white shadow-xl shadow-zinc-900/10 ring-1 ring-zinc-200"
        role="menu"
    >
        ...
        <div class="max-h-80 overflow-y-auto [scrollbar-width:thin]">
            @forelse ($this->items as $n)
                <button
                    ...
                    role="menuitem"
                >
                    <span class="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-blue-600">

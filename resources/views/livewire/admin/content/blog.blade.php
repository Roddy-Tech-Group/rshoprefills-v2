...
class extends Component {
    ...
    public function edit(int $id): void
    {
        ...
        // Body is stored as an array of paragraphs - join with blank lines so
        // editors see one paragraph per double-newline in the textarea.
...
<div class="w-full px-4 py-8 sm:px-6 lg:px-8">
    ...
    <header class="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
            ...
            <p class="mt-1 text-sm text-zinc-600">CMS-managed articles shown at <a href="/blog" target="_blank" class="text-blue-600 hover:underline">/blog</a>.</p>
        </div>
                        ...
                        <tr class="hover:bg-zinc-50">
                            ...
                            <td class="px-5 py-3 text-right">
                                <div class="inline-flex items-center gap-1.5">
                                    <button wire:click="togglePublish({{ $post->id }})" type="button" class="rounded-[10px] bg-zinc-100 px-2.5 py-1 text-[11px] font-semibold text-zinc-700 transition-colors hover:bg-zinc-200">{{ $post->is_published ? 'Unpublish' : 'Publish' }}</button>
                                    <button wire:click="edit({{ $post->id }})" type="button" class="rounded-[10px] bg-blue-50 px-2.5 py-1 text-[11px] font-semibold text-blue-700 transition-colors hover:bg-blue-100">Edit</button>
                                    <button wire:click="delete({{ $post->id }})" wire:confirm="Delete this post permanently?" type="button" class="rounded-[10px] bg-red-50 px-2.5 py-1 text-[11px] font-semibold text-red-700 transition-colors hover:bg-red-100">Delete</button>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-12 text-center text-sm text-zinc-600">No posts yet. Click "New post" to create the first one.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
            ...
            <form wire:submit="save" class="relative max-h-[90vh] w-full max-w-2xl overflow-hidden rounded-[10px] bg-white shadow-2xl flex flex-col">
                <div class="flex shrink-0 items-start justify-between gap-4 border-b border-zinc-100 px-5 py-4">
                    <h3 class="text-sm font-bold text-zinc-900">{{ $editingId ? 'Edit post' : 'New post' }}</h3>
                ...
                <div class="space-y-4 overflow-y-auto px-5 py-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input wire:model="category" type="text" placeholder="e.g. Product" class="mt-1.5 w-full rounded-[10px] border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15">
                    ...
                    <div>
                        ...
                        <textarea wire:model="excerpt" rows="2" placeholder="Short summary shown on the blog index." class="mt-1.5 w-full rounded-[10px] border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15"></textarea>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-[10px] font-semibold uppercase tracking-wider text-zinc-800">Author</label>
                            <input wire:model="author" type="text" class="mt-1.5 w-full rounded-[10px] border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15">
                            @error('author') <p class="mt-1 text-[11px] font-medium text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="text-[10px] font-semibold uppercase tracking-wider text-zinc-800">Read time</label>
                            <input wire:model="readTime" type="text" placeholder="e.g. 4 min read" class="mt-1.5 w-full rounded-[10px] border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15">
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-[10px] font-semibold uppercase tracking-wider text-zinc-800">Image filename</label>
                            <input wire:model="image" type="text" placeholder="e.g. hero-launch.png" class="mt-1.5 w-full rounded-[10px] border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15">

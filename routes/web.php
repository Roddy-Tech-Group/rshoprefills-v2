...
Route::get('gift-cards/{brandSlug}', function (string $brandSlug) {
    ...
    $brandKey = \Illuminate\Support\Facades\Cache::tags(['catalog'])->remember("brand_slug_{$brandSlug}", 3600, function () use ($brandSlug) {
    ...
    $product = \Illuminate\Support\Facades\Cache::tags(['catalog'])->remember("brand_product_{$brandKey}_{$requested}", 3600, function () use ($brandKey, $requested) {
        $p = Product::query()
...
Route::get('esims/{slug}', function (string $slug) {
    $product = \Illuminate\Support\Facades\Cache::tags(['catalog'])->remember("esim_product_{$slug}", 3600, function () use ($slug) {
        return Product::query()
            ->where('slug', $slug)
            ->where('is_active', true)
...
Route::get('topups/{brandSlug}', function (string $brandSlug) {
    ...
    $brandKey = \Illuminate\Support\Facades\Cache::tags(['catalog'])->remember("topup_brand_{$brandSlug}", 3600, function () use ($brandSlug) {
        return Product::query()
            ->whereNotNull('brand_key')
            ->where('is_active', true)
    ...
    $product = \Illuminate\Support\Facades\Cache::tags(['catalog'])->remember("topup_product_{$brandKey}_{$requested}", 3600, function () use ($brandKey, $requested) {
...
Route::get('bills/{brandSlug}', function (string $brandSlug) {
    ...
    $brandKey = \Illuminate\Support\Facades\Cache::tags(['catalog'])->remember("bill_brand_{$brandSlug}", 3600, function () use ($brandSlug) {
        return Product::query()
            ->whereNotNull('brand_key')
            ->where('is_active', true)
    ...
    $product = \Illuminate\Support\Facades\Cache::tags(['catalog'])->remember("bill_product_{$brandKey}_{$requested}", 3600, function () use ($brandKey, $requested) {
        $base = fn () => Product::query()
            ->where('brand_key', $brandKey)
            ->where('is_active', true)
            ->whereHas('category', fn ($q) => $q->where('slug', 'bill-payments'))
